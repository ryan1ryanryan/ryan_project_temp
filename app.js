...
async function init() {
    ...
    // Retrieve and clean keywords: lowercase and trim whitespace
    ...
    try {
        status.innerText = "Loading AI-Ranked Feed...";
        const response = await fetch('./news.json');
        const data = await response.json();
        let articles = data.articles || [];

        // 1. FILTER: Remove items if ANY part of the text contains an ignored string
        let workingList = articles.filter(article => {
            const text = `${article.title} ${article.description} ${article.source?.name || ''}`.toLowerCase();
            // Substring match: If "verge" is inside "The Verge", it returns true and flips to false for filter
            return !USER_IGNORED.some(word => text.includes(word));
        });

        // 2. SORT: Use substring matching to move interests to the top
        workingList.sort((a, b) => {
            const aText = `${a.title} ${a.description} ${a.source?.name || ''}`.toLowerCase();
            const bText = `${b.title} ${b.description} ${b.source?.name || ''}`.toLowerCase();
        ...
        status.innerText = `Displaying ${workingList.length} articles. (Updated: ${data.last_updated})`;
        
    } catch (err) { 
        status.innerText = "Error: news.json not found."; 
    }
}
